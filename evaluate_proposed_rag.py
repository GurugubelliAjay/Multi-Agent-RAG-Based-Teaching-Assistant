...
def run_proposed_evaluation():
    ...
    data_proposed = {"user_input": [], "response": [], "retrieved_contexts": [], "reference": []}
    
    print(f"\n--- Gathering Responses for Proposed RAG ---")
    for tc in test_cases:
        ...
        # --- META-COMMENTARY SCRUBBING FOR RAGAS ---
        # Remove partial refusals so Ragas doesn't treat them as hallucinated claims
        eval_response = re.sub(r"(?i)\n?\*?\*?Note:\*?\*? your uploaded notes do not cover[^\.]*\.", "", response)
        
        # Map full refusals to a standard "I don't know" 
        # Ragas is natively trained to extract 0 claims from "I don't know", preventing a Faithfulness penalty.
        if "I'm sorry, but I cannot find the answer" in eval_response:
            eval_response = "I don't know."
            
        eval_response = eval_response.strip()

        data_proposed["user_input"].append(query)
        data_proposed["response"].append(eval_response)
    ...
    print(f"\nStarting Ragas Bulk Evaluation for Proposed RAG... (This may take a minute)")
    all_results = []
    for i in range(len(dataset_proposed)):
        single_row = dataset_proposed.select([i])
        print(f"\nEvaluating Proposed RAG - Question {i+1}/{len(dataset_proposed)}...")
        ...
        if all_results:
            df_proposed = pd.concat(all_results, ignore_index=True)
            df_proposed.to_csv("ragas_proposed_results.csv", index=False)
            print("--> Progressively saved Proposed RAG results to 'ragas_proposed_results.csv'")
    
    df_proposed = pd.concat(all_results, ignore_index=True) if all_results else pd.DataFrame()

    print("\n--- EVALUATION RESULTS ---")
    if not df_proposed.empty:
        print("Detailed results saved to 'ragas_proposed_results.csv'.")
        metrics_to_compare = ["faithfulness", "answer_relevancy", "context_precision", "context_recall"]
        print("\n=== PROPOSED RAG SUMMARY ===")
        print(f"Total Questions Evaluated: {len(df_proposed)}")
        for m in metrics_to_compare:
            if m in df_proposed.columns:
                val = df_proposed[m].mean()
                print(f"{m.replace('_', ' ').title():<25} | {val:<12.4f}")
        print("=========================\n")
        
        # --- ANSWERED-ONLY METRICS ---
        answered_df = df_proposed[df_proposed['response'] != "I don't know."]
        print(f"\n=== PROPOSED RAG (Excluding Safe Refusals) ===")
        print(f"Total Questions Answered: {len(answered_df)} / {len(df_proposed)}")
        for m in metrics_to_compare:
            if m in answered_df.columns:
                val = answered_df[m].mean()
                print(f"{m.replace('_', ' ').title():<25} | {val:<12.4f}")
        print("=========================\n")
    else:
