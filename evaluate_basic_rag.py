...
from agent_tools import get_embeddings, get_rag_chain
...
def run_basic_evaluation():
    ...
    if not rag_chain:
        print("Error: Could not load RAG chain. Ensure the DB is initialized.")
        return

    data_basic = {"user_input": [], "response": [], "retrieved_contexts": [], "reference": []}
    
    print(f"\n--- Gathering Responses for Basic RAG ---")
    ...
    print(f"\nStarting Ragas Bulk Evaluation for Basic RAG... (This may take a minute)")
    all_results = []
    for i in range(len(dataset_basic)):
        single_row = dataset_basic.select([i])
        print(f"\nEvaluating Basic RAG - Question {i+1}/{len(dataset_basic)}...")
        ...
        if all_results:
            df_basic = pd.concat(all_results, ignore_index=True)
            df_basic.to_csv("ragas_basic_results.csv", index=False)
            print("--> Progressively saved Basic RAG results to 'ragas_basic_results.csv'")
    
    df_basic = pd.concat(all_results, ignore_index=True) if all_results else pd.DataFrame()

    print("\n--- EVALUATION RESULTS ---")
    if not df_basic.empty:
        print("Detailed results saved to 'ragas_basic_results.csv'.")
